...
export function Snowflake({ winterRef }: any) {
  ...
  useEffect(() => {
    ...
    gsap.to(snowflakeRef.current, { opacity: 0, scrollTrigger: { end: 'center -10%', start: 'center 40%', scrub: true, trigger: winterRef.current, }, })
    gsap.from(snowflakeRef.current, { opacity: 0, scrollTrigger: { end: 'center 60%', start: 'center 110%', scrub: true, trigger: winterRef.current, }, })
	})

  return (
    <div className='align-items-center d-flex justify-content-center position-fixed start-0 top-0' ref={ snowflakeRef } style={{ height: 0, width: 0, zIndex: -1, }}>
